/**
 * ByFlow POS — API Routes (HARDENED)
...
function registerRoutes(app) {
  ...
  app.put('/pos/tables/:id/status', posJson, (req, res) => {
    ...
    params.push(tableId);
    db.prepare(`UPDATE tables SET ${setClauses.join(', ')} WHERE id = ?`).run(...params);
    res.json({ ok: true });
  });
  ...
  app.post('/pos/products', posJson, (req, res) => {
    ...
    const result = db.prepare(
      'INSERT INTO products (name, category_id, price, cost, icon, stock, happy_hour, hh_discount) VALUES (?,?,?,?,?,?,?,?)'
    ).run(
      sanitize(name, 100),
  ...
  app.post('/pos/orders', posJson, (req, res) => {
    ...
    const createOrder = db.transaction(() => {
      // Check table is libre first
      const table = db.prepare('SELECT status FROM tables WHERE id = ?').get(tid);
  ...
  app.get('/pos/orders/table/:tableId', (req, res) => {
    ...
    const order = db.prepare(`
      SELECT o.*, t.number as table_number, e.name as waiter_name
      FROM orders o JOIN tables t ON o.table_id = t.id JOIN employees e ON o.waiter_id = e.id
      WHERE o.table_id = ? AND o.status = 'abierta' ORDER BY o.created_at DESC LIMIT 1
    `).get(tableId);
  ...
  app.post('/pos/payments', posJson, (req, res) => {
    ...
    const processPayment = db.transaction(() => {
      const order = db.prepare('SELECT table_id, status FROM orders WHERE id = ?').get(oid);
      ...
      ).run(oid, method, amt, cashRcv, change, tipAmt, cid);

      db.prepare("UPDATE orders SET status = 'pagada', closed_at = datetime('now'), tip = ? WHERE id = ?").run(tipAmt, oid);

      db.prepare(
        "UPDATE tables SET status = 'libre', current_order_id = NULL, waiter_id = NULL, guests = 0, opened_at = NULL WHERE id = ?"
      ).run(order.table_id);
  ...
  app.get('/pos/kitchen', (req, res) => {
    ...
    const items = db.prepare(`
      ...
      WHERE oi.status IN ('enviado', 'preparando') AND o.status = 'abierta'
  ...
  app.post('/pos/covers', posJson, (req, res) => {
    ...
    ).run(type || 'general', g, parseFloat(amount) || 0, parseInt(security_id) || null, sanitize(notes, 200));
  ...
  app.get('/pos/covers/today', (req, res) => {
    ...
    const covers = db.prepare(`
      ...
      WHERE date(c.created_at) = date('now') ORDER BY c.created_at DESC
    `).all();
  ...
  app.get('/pos/karaoke/queue', (req, res) => {
    ...
    let query = `SELECT kq.*, t.number as table_number FROM karaoke_queue kq
      LEFT JOIN tables t ON kq.table_id = t.id WHERE kq.status IN ('espera', 'cantando')`;
    const params = [];
  ...
  app.post('/pos/karaoke/queue', posJson, (req, res) => {
    ...
    ).run(parseInt(table_id) || null, sanitize(song_title, 200), sanitize(singer_name, 100), maxPos + 1);
  ...
  app.post('/pos/tables', posJson, (req, res) => {
    ...
    const result = db.prepare(
      'INSERT INTO tables (number, area, capacity) VALUES (?,?,?)'
    ).run(sanitize(number, 10), a, parseInt(capacity) || 4);
  ...
  app.get('/pos/reports/today', (req, res) => {
    ...
    const sales = db.prepare(`
      ...
      FROM orders WHERE date(created_at) = date('now') AND status = 'pagada'
    `).get();
  ...
  app.put('/pos/settings', posJson, (req, res) => {
    ...
    const update = db.transaction((items) => {
      for (const [k, v] of items) stmt.run(k, sanitize(String(v), 500));
    });
    update(entries);
  ...
  app.get('/pos/happy-hour/active', (req, res) => {
    ...
    const active = db.prepare(`
      SELECT * FROM happy_hours WHERE active = 1
...
function recalcOrder(db, orderId) {
  ...
  const taxRow = db.prepare("SELECT value FROM bar_settings WHERE key = 'tax_rate'").get();
